...
t  = datetime.datetime(2016,11,20,00,00,00)
dt = datetime.timedelta(hours=24)
img_begin = get_normalized_image_variable(t)
img_end   = get_normalized_image_variable(t+dt)
z_begin = model.encode(img_begin)
z_end   = model.encode(img_end)

n=10
for i in range(n+1):
    r = float(i)/n
    z = z_begin * (1-r) + z_end * r
    img_intp = model.decode(z)
    plot_sun_image(img_intp.data[0,0], "image-interporate-{:02}.png".format(i), title = '{}'.format(t+dt*i/n))
