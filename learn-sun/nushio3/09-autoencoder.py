...
while True:
    ...
    if vizualization_mode: plot_sun_image(img_input.data[0,0], "image-auto-input.png", title = 'input')

    img_observed = img_input

    img_predicted = model(img_input)
    if vizualization_mode: plot_sun_image(img_predicted.data[0,0], "image-auto-predicted.png", title = 'train {}th epoch'.format(epoch))

    loss = F.sqrt(F.sum((img_predicted - img_observed)**2))
    model.cleargrads()
    loss.backward()
    opt.update()
    serializers.save_npz('sun-autoencoder-211.model', model)
