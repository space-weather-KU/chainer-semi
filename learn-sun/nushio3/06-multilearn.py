...
img_input_2 = get_normalized_image_variable(dt2 * training_data_size + t)
plot_sun_image(img_input_2.data[0,0], "image-test-input.png", title = 'test input')

img_observed_2 = get_normalized_image_variable(dt2 * training_data_size + t+dt)
...
while True:
    for i in range(training_data_size):
        img_predicted = model(img_input[i])
        loss = F.sqrt(F.sum((img_predicted - img_observed[i])**2))
        model.cleargrads()
        loss.backward()
        opt.update()

    if epoch%10 ==0:
        plot_sun_image(img_predicted.data[0,0], "image-train-{}-predicted.png".format(i), title = 'train {}th epoch'.format(epoch))
